...
for(basePhen in 1:length(base.phenotypes.names)){
	...
	if(dosage){
print(user.covariate.file)
print(covariates)
print(is.na(user.covariate.file))
	  ...
	  if(dos.impute2){
	    ...
	    dos.format <- 3	
          ...
          target.map <- dos.sep.map # add map file
	  if(!dos.fam.is.samp){
	  ...
	  #if(covary & !is.na(user.covariate.file)){  <- looks like a bug
	...
	if(clump.snps & dosage & dosage.format == "gen"){
	  ## clump and extract independant snps
	  system(paste(plink," --noweb --dosage ", gen.name, " noheader format=",dos.format," dose",dos.coding," skip0=",dos.skip0," skip1=",dos.skip1," --fam ", target.fam," --map ", target.map,"  --clump cleaned_base.assoc --clump-p1 ",clump.p1," --clump-p2 ",clump.p2," --clump-r2 ",clump.r2," --clump-kb ",clump.kb," --out cleaned_base", sep=""))
	  system("tail -n +2 cleaned_base.clumped | awk '{print $3}'  | sort -k1,1 | awk '($1 != \"\"){print}'  > LE_SNPs")
	  ## linkage equilibrium snps
	  if(!use.beta){
	  system(paste("tail -n +2 cleaned_base.assoc | sort -k1,1 | join -1 1 -2 1 LE_SNPs ``-'' | awk '{print $",which(head.disc=="SNP"),
	...
	if(prune.snps & dosage & dosage.format == "gen"){
	  ## prune and extract independant snps
	  system(paste(plink," --noweb --dosage ", gen.name, " noheader format=",dos.format," dose",dos.coding," skip0=",dos.skip0," skip1=",dos.skip1," --fam ", target.fam," --map ", target.map,"  --indep-pairwise ",prune.kb.wind," ",prune.kb.step," ",prune.kb.r2," --out cleaned_base", sep=""))
	  system("tail -n +2 cleaned_base.clumped | awk '{print $3}'  | sort -k1,1 | awk '($1 != \"\"){print}'  > LE_SNPs")
	  ## linkage equilibrium snps
	  if(!use.beta){
	  system(paste("tail -n +2 cleaned_base.assoc | sort -k1,1 | join -1 1 -2 1 LE_SNPs ``-'' | awk '{print $",which(head.disc=="SNP"),
 ...
 system(paste(plink," --noweb --dosage ", gen.name, " noheader format=",dos.format," dose",dos.coding," skip0=",dos.skip0," skip1=",dos.skip1," --fam ", target.fam," --map ", target.map,"  --score rawfile.raw --q-score-file rangelist_ranges --q-score-range rangelist.txt   --out PROFILES", sep = ""))
 }
     if(dosage & is.na(dos.path.to.lists) & is.na(dos.list.file) & dos.coding == 2){
           system(paste(plink," --noweb --dosage ", gen.name, " noheader format=",dos.format," skip0=",dos.skip0," skip1=",dos.skip1," --fam ", target.fam," --map ", target.map,"  --score rawfile.raw --q-score-file rangelist_ranges --q-score-range rangelist.txt   --out PROFILES", sep = ""))
     }
	...
	if(dosage & (!is.na(dos.path.to.lists) | !is.na(dos.list.file))){
	  for(i in 1:22){
	    ...
	    system(paste(plink," --noweb --dosage ",gen.name," noheader format=",dos.format," dose",dos.coding," skip0=",dos.skip0," skip1=",dos.skip1," --fam ", target.fam," --map ", target.map,"  --score rawfile.raw --q-score-file rangelist_ranges --q-score-range rangelist.txt   --out ",i,"PROFILES", sep = ""))  
	  }
	}            
...
# FY: not intend to use this portion for now
if(sumsum){
